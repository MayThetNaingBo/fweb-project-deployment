...
router.post("/events/:id/register-request", async (req, res) => {
    ...
    try {
        ...
        if (
            ...
            console.warn("Already registered or requested for memberId:", memberId);
            return res.status(400).json({ error: "Already registered or requested." });
        }
        ...
        res.status(200).json({ success: true, message: "Registration request sent." });
    } catch (err) {
