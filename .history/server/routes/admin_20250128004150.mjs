...
router.post("/add/members", async (req, res) => {
    ...
    try {
        ...
        const verificationLink = `http://localhost:5173/member/create-password?token=${token}`;
        ...
        console.error("Error in /add/members:", error); // Log for debugging
...
router.get("/event/:id/requests", async (req, res) => {
    try {
        const event = await Event.findById(req.params.id).populate({
            path: "registrationRequests",
            select: "name email school", // Select only required fields
        });
        if (!event) return res.status(404).json({ error: "Event not found" });
...
router.post("/event/:id/approve-request", async (req, res) => {
    const { userId } = req.body;

    try {
        console.log("Approving request for:", { eventId: req.params.id, userId });
        ...
        if (event.members.includes(memberId)) {
            console.warn("Member already added:", userId);
            return res.status(400).json({ error: "Member already approved." });
        }

        // Add to members and remove from registrationRequests
        event.members.addToSet(memberId); // Avoid duplicates
        event.registrationRequests.pull(memberId);

        // Save changes
        await event.save();

        console.log("Approval successful for:", userId);
        res.json({
            success: true,
            message: "Member approved and added to the event.",
        });
    } catch (err) {
...
router.post("/event/:id/reject-request", async (req, res) => {
    const { userId } = req.body;
