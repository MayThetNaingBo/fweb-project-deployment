...
function isAuthenticated(role) {
    // Example logic: Check for authToken and role
    const token = localStorage.getItem("authToken");
    const userRole = localStorage.getItem("userRole"); // Example: 'admin' or 'member'

    return token && userRole === role; // Ensure token exists and matches the required role
}

// Define routes with authentication check inline
const routes = [
    {
        path: "/",
        element: <Layout />,
    ...
    {
        path: "/admin",
        element: isAuthenticated("admin") ? <AdminLayout /> : <Navigate to="/authentication" replace />,
        children: [
            ...
            { path: "/admin/event/:id/select-members", element: <SelectMembers /> },
    ...
    {
        path: "/member",
        element: isAuthenticated("member") ? <MemberLayout /> : <Navigate to="/authentication" replace />,
        children: [
